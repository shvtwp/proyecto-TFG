...
cualquier entorno, es fundamental contar con un gestor de paquetes. 
Este permitirá instalar y documentar todas las dependencias necesarias de forma automática,
...
\begin{itemize}
    \item Sea compatible con la versión de Python utilizada en el proyecto.
    \item Permita aislar el entorno de desarrollo para evitar conflictos con otras instalaciones.
    \item Sea fácil de usar y mantener en proyectos pequeños o medianos.
...
\begin{itemize}
    \item La simplicidad y facilidad de aprendizaje.
    \item Que sea una herramienta estándar y ampliamente conocida.
\end{itemize}

\subsubsection{Herramientas consideradas}
Se han comparado tres opciones:
\begin{itemize}
    \item \textbf{pip + venv}: Herramientas oficiales incluidas con Python que permiten instalar
     paquetes y crear entornos virtuales de manera sencilla.
    \item \textbf{pipenv}: Integra la gestión de dependencias y del entorno virtual en un solo
    comando, empleando un archivo \texttt{Pipfile}.
    \item \textbf{poetry}: Más avanzado, orientado a proyectos que quieren distribuirse como 
    paquetes, opera con \texttt{pyproject.toml} para la gestión.
\end{itemize}

\subsubsection{Herramienta seleccionada}
Se ha seleccionado \textbf{pip} junto con \textbf{venv} por ser la opción más simple, oficial y 
suficiente para las necesidades actuales del proyecto. 
Aunque \textbf{pipenv} ofrece la ventaja de integrar en un único comando la gestión del entorno 
virtual y de dependencias, añade una capa de complejidad innecesaria para un proyecto que, por 
ahora, únicamente requiere definir unas pocas dependencias básicas. 
Al ser herramientas incluidas directamente en Python, \textbf{pip} y \textbf{venv} también 
garantizan mayor compatibilidad y menor curva de aprendizaje, lo que facilita entender y usar 
el proyecto sin configuraciones adicionales.