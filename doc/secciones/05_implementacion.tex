...
La implementación del software se ha dividido en hitos. Estos han sido definidos en \href{https://github.com/shvtwp/proyecto-TFG}{GitHub}
...
Una vez fallaron las pruebas iniciales, se implementó el código mínimo necesario para hacerlas pasar: 
un único archivo \texttt{ui\_web.py} con la aplicación Flask, tres rutas y respuestas HTML estáticas sin 
plantillas ni dependencias adicionales, manteniendo el código al mínimo indispensable.

El resultado es una interfaz web mínima aunque funcional que puede ejecutarse con:

\begin{verbatim}
poetry run flask --app heraldica/ui_web.py --debug run
\end{verbatim}

Posteriormente, con la estructura básica en marcha, se procedió a integrar la base de datos y la lógica 
de negocio.

\subsubsection{Integración con la base de datos y capa de dominio}
Para conectar la interfaz con los datos reales del catálogo se añadió un nuevo módulo,
\texttt{heraldica/ui\_catalogo.py}, que actúa como adaptador entre la capa de dominio y la interfaz web.
Su función es delegar en la clase \texttt{Catalogo} las operaciones de búsqueda y filtrado, pero 
devolviendo los resultados en una estructura más simple y apta para renderizado.
De esta forma, cada entidad del dominio se convierte en un diccionario de Python con valores de texto 
plano.

El adaptador incluye un método llamado \texttt{buscar libre}, que combina los distintos criterios de filtrado
del catálogo en una única búsqueda global. Además, si no se recibe ningún término de búsqueda, el método
devuelve el catálogo completo, lo que permite explorar todos los escudos sin necesidad de especificar 
criterios.

A continuación, el módulo \texttt{ui\_web.py} se amplió para incorporar la integración con este adaptador.
La ruta \texttt{/search} obtiene los resultados mediante \texttt{CatalogoUI} y genera dinámicamente
una lista de tarjetas HTML.  
Cada tarjeta incluye el nombre del municipio, una miniatura clicable con la imagen del escudo
y una serie de distintivos (\emph{badges}) con los atributos más relevantes:
campo, muebles, pieza heráldica, adorno y portador.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figuras/basicUI.png}
    \caption{Interfaz web inicialmostrando resultados de búsqueda.}
    \label{fig:interfaz-basica}
\end{figure}

En esta fase no se emplean plantillas ni JavaScript, siguiendo un enfoque \textit{server-side} puro
que prioriza la transparencia y la sencillez del código.  
El diseño se mantiene intencionadamente minimalista, con un HTML básico generado desde la vista
para facilitar la validación y la posterior incorporación de plantillas.

Una vez todas las pruebas pasaron, la interfaz permite realizar búsquedas reales sobre la base de datos,
visualizar los resultados con sus atributos y, en ausencia de filtro, listar el catálogo completo.
Este avance completa la conexión entre la capa de presentación y el modelo de dominio,
dejando la aplicación en un estado funcional y demostrable.

\subsubsection{Búsqueda combinada con filtros}
Una vez implementada la visualización básica de resultados, se añadió la funcionalidad de búsqueda 
combinada mediante múltiples criterios simultáneos. El objetivo es permitir a \hyperref[sec:hu1]{HU1} 
y \hyperref[sec:hu2]{HU2} refinar los resultados aplicando varios filtros de forma conjunta, reduciendo 
el conjunto de escudos mostrados únicamente a aquellos que cumplen todas las condiciones especificadas.

Para ello, se implementó el método par a la búsqueda combinada en \texttt{CatalogoUI}, que acepta 
cuatro parámetros opcionales:

\begin{itemize}
    \item \textbf{texto}: búsqueda libre que filtra por nombre, portador o provincia.
    \item \textbf{esmalte}: filtra escudos por el color del campo.
    \item \textbf{mueble}: filtra por la presencia de un mueble heráldico específico.
    \item \textbf{adorno}: filtra por el adorno exterior.
\end{itemize}

La implementación aplica los filtros de manera secuencial sobre el conjunto de 
resultados, reduciendo progresivamente la lista hasta obtener solo aquellos escudos que satisfacen 
todas las condiciones. Únicamente se aplican los filtros que tienen valor, permitiendo combinaciones 
flexibles.

Para evitar duplicar datos en el código, las listas de valores disponibles para los filtros se 
cargan dinámicamente desde los archivos JSON de configuración ubicados en el directorio \texttt{data/}.
Esto mantiene la separación entre datos y lógica, facilitando futuras ampliaciones del catálogo 
sin necesidad de modificar el código fuente.

La interfaz web se actualizó para aceptar parámetros de búsqueda mediante la URL. Con esta funcionalidad
implementada, la interfaz permite realizar búsquedas precisas y complejas sobre el catálogo, mejorando 
significativamente la utilidad del sistema para los usuarios.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figuras/busquedaCombinada.png}
    \caption{URL y resultados de la búsqueda combinada.}
    \label{fig:busqueda-combinada}
\end{figure}

\subsubsection{Implementación de plantillas y diseño visual}
Una vez validada la funcionalidad de búsqueda combinada, se procedió a mejorar la presentación 
visual de la interfaz, sustituyendo la generación manual de HTML por un sistema de plantillas 
estructurado y aplicando un diseño moderno y responsive.

Hasta este punto, la interfaz generaba todo el HTML dinámicamente desde el código Python, 
concatenando cadenas de texto para construir los resultados. Este enfoque, aunque funcional, presenta 
varios inconvenientes: mezcla lógica de presentación con lógica de negocio, dificulta el mantenimiento 
del código HTML, y complica la incorporación de estilos y comportamientos interactivos.

Para resolver estas limitaciones, se adoptó el motor de plantillas \textbf{Jinja2}, que viene integrado 
de forma nativa con Flask. Este sistema permite separar completamente la estructura HTML de la lógica 
Python, facilitando la lectura, el mantenimiento y la evolución del código.

\paragraph{Estructura de plantillas}
Se diseñó una jerarquía de plantillas basada en herencia para evitar duplicación de código:

\begin{itemize}
    \item \textbf{base.html}: plantilla base que define la estructura común de todas las páginas 
    (cabecera, estilos, scripts, pie de página). Incluye la carga de Pico.css para los estilos 
    y HTMX para la interactividad dinámica.
    \item \textbf{index.html}: extiende \texttt{base.html} y define el contenido principal de 
    la aplicación: el formulario de búsqueda con sus filtros y el contenedor donde se mostrarán 
    los resultados.
    \item \textbf{resultados.html}: plantilla reutilizable que genera únicamente el grid de 
    tarjetas con los escudos encontrados.
\end{itemize}

\paragraph{Diseño visual con Pico.css}
Para conseguir una apariencia profesional sin invertir tiempo en diseñar CSS desde cero, se 
integró \textbf{Pico.css}, una hoja de estilos minimalista sin clases que aplica automáticamente 
un diseño limpio y accesible a los elementos HTML semánticos.

Además de los estilos base proporcionados por Pico.css, se añadieron estilos personalizados para:

\begin{itemize}
    \item \textbf{Grid responsive}: configuración de una cuadrícula que muestra una columna en 
    dispositivos móviles y dos columnas en pantallas de escritorio, adaptándose automáticamente 
    al tamaño de la ventana.
    \item \textbf{Fichas de escudos}: cada escudo se presenta en una ficha que incluye una 
    imagen de 150×150 píxeles y sus atributos principales.
    \item \textbf{Área de scroll independiente}: la zona de resultados tiene scroll propio, 
    manteniendo fijo el formulario de búsqueda en la parte superior y el pie de página en la 
    inferior, mejorando la usabilidad al explorar catálogos extensos.
\end{itemize}

\paragraph{Interactividad con HTMX}
La integración de HTMX se realizó añadiendo tres atributos al formulario de búsqueda:

\begin{itemize}
    \item \texttt{hx-get="/search"}: indica que al enviar el formulario se debe realizar una 
    petición GET a la ruta \texttt{/search}.
    \item \texttt{hx-target="\#resultados"}: especifica que la respuesta debe insertarse en el 
    elemento con ID \texttt{resultados}, actualizando únicamente esa sección de la página.
    \item \texttt{hx-indicator="\#loading"}: muestra un indicador de carga mientras se procesa 
    la petición, proporcionando feedback visual al usuario.
\end{itemize}

Con esta configuración, al efectuar una búsqueda, HTMX intercepta el envío del formulario, 
efectúa la petición de forma asíncrona y actualiza solo la zona de resultados sin recargar 
toda la página. Esto proporciona una experiencia fluida y moderna, similar a las aplicaciones 
de página única (SPA), pero manteniendo la simplicidad de renderizado en el servidor.

La ruta \texttt{/search} se adaptó para devolver únicamente el fragmento HTML de los resultados 
(\texttt{resultados.html}) en lugar de la página completa, acelerando así la respuesta.

El resultado es una interfaz web funcional, visualmente atractiva y con una experiencia de 
usuario moderna, que permite hacer búsquedas complejas de manera rápida e intuitiva, 
manteniendo la sencillez arquitectónica del sistema.

\subsubsection{Capturas de funcionamiento}

A continuación, se muestran capturas de pantalla que ilustran el funcionamiento de la interfaz 
con diferentes combinaciones de filtros de búsqueda:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figuras/busquedaValle.png}
    \caption{Resultados de búsqueda libre por texto: ``valle'' (vista escritorio).}
    \label{fig:busqueda-valle}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figuras/busquedaValleSinople.png}
    \caption{Resultados de búsqueda combinada: texto ``valle'' + esmalte ``sinople'' (vista escritorio).}
    \label{fig:busqueda-valle-sinople}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figuras/busquedaValleAzurLlave.png}
    \caption{Resultados de búsqueda combinada: texto ``valle'' + esmalte ``azur'' + mueble ``llave'' (vista escritorio).}
    \label{fig:busqueda-valle-azur-llave}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{figuras/busquedaBornosMovil.png}
    \caption{Resultados de búsqueda por texto: ``bornos'' (vista móvil).}
    \label{fig:busqueda-bornos-movil}
\end{figure}